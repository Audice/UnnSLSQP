...
void SLSQP_Solver::dcopy___(int *n_, const double *dx, int incx, 
		     double *dy, int incy)
{
     int i, n = *n_;
...
void SLSQP_Solver::daxpy_sl__(int *n_, const double *da_, const double *dx,
...
double SLSQP_Solver::ddot_sl__(int *n_, double *dx, int incx, double *dy, int incy)
{
     int n = *n_, i;
     double sum = 0;
     if (n <= 0) return 0;
...
double SLSQP_Solver::dnrm2___(int *n_, double *dx, int incx)
{
     int i, n = *n_;
     double xmax = 0, scale;
     double sum = 0;
...
void SLSQP_Solver::dsrot_(int n, double *dx, int incx,
...
void SLSQP_Solver::dsrotg_(double *da, double *db, double *c, double *s)
{
...
void SLSQP_Solver::dscal_sl__(int *n_, const double *da, double *dx, int incx)
{
     int i, n = *n_;
     double alpha = *da;
...
void SLSQP_Solver::h12_(const int *mode, int *lpivot, int *l1,
...
void SLSQP_Solver::nnls_(double *a, int *mda, int *m, int *
...
void SLSQP_Solver::ldp_(double *g, int *mg, int *m, int *n,
...
void SLSQP_Solver::lsi_(double *e, double *f, double *g,
...
void SLSQP_Solver::hfti_(double *a, int *mda, int *m, int *
...
void SLSQP_Solver::lsei_(double *c__, double *d__, double *e,
...
void SLSQP_Solver::lsq_(int *m, int *meq, int *n, int *nl,
...
/* L40: */
    ...
    for (i__ = 1; i__ <= *n; ++i__)
	 if (!LocalOptHelper::nlopt_isinf(xl[i__])) w[(ip - i__1) + i__ * i__1] = +1.0;
...
/* L50: */
    ...
    /* SGJ, 2010: skip constraints for infinite bounds */
    for (i__ = 1; i__ <= *n; ++i__)
	 if (!LocalOptHelper::nlopt_isinf(xu[i__])) w[(im - i__1) + i__ * i__1] = -1.0;
...
/*  AUGMENT VECTOR H BY XL AND XU */
    ...
    /* SGJ, 2010: skip constraints for infinite bounds */
    for (i__ = 1; i__ <= *n; ++i__) {
	 w[(il-1) + i__] = LocalOptHelper::nlopt_isinf(xl[i__]) ? 0 : xl[i__];
	 w[(iu-1) + i__] = LocalOptHelper::nlopt_isinf(xu[i__]) ? 0 : -xu[i__];
    }
...
void SLSQP_Solver::ldl_(int *n, double *a, double *z__,
...
void SLSQP_Solver::slsqpb_(int *m, int *meq, int *la, int *
...
L190:
    ...
    *mode = line == 1 ? -2 : 1;
    goto L330;
L200:
    if (LocalOptHelper::nlopt_isfinite(h1)) {
...
/*                              optimizer                               * */
/* *********************************************************************** */
void SLSQP_Solver::slsqp(int *m, int *meq, int *la, int *n,
...
void SLSQP_Solver::length_work(int *LEN_W, int *LEN_JW, int M, int MEQ, int N)
{
...
nlopt_result SLSQP_Solver::nlopt_slsqp(unsigned n, nlopt_func f, void *f_data,
			 ...
			 nlopt_stopping *stop)
{
     ...
     unsigned mtot = LocalOptHelper::nlopt_count_constraints(m, fc);
     unsigned ptot = LocalOptHelper::nlopt_count_constraints(p, h);
     ...
     max_cdim = MAX2(LocalOptHelper::nlopt_max_constraint_dim(m, fc),
         LocalOptHelper::nlopt_max_constraint_dim(p, h));
...
#define U(n) ((unsigned) (n))
     ...
     do {
	  ...
	  case 1:{ /* don't need grad unless we don't have it yet */
	      ...
	      ++ *(stop->nevals_p);
	      if (LocalOptHelper::nlopt_stop_forced(stop)) {
		  fcur = HUGE_VAL; ret = NLOPT_FORCED_STOP; goto done; }
	      if (LocalOptHelper::nlopt_isfinite(fcur)) {
		  want_grad = 0;
		  ii = 0;
		  for (i = 0; i < p; ++i) {
		      unsigned j, k;
              LocalOptHelper::nlopt_eval_constraint(c+ii, newcgrad, h+i, n, xcur);
		      if (LocalOptHelper::nlopt_stop_forced(stop)) {
		  ...
		  for (i = 0; i < m; ++i) {
		      unsigned j, k;
              LocalOptHelper::nlopt_eval_constraint(c+ii, newcgrad, fc+i, n, xcur);
		      if (LocalOptHelper::nlopt_stop_forced(stop)) {
			  ret = NLOPT_FORCED_STOP; goto done; }
		  ...
		  if (feasible_cur) {
		      ...
		      stop->ftol_abs *= 10;
		      stop->xtol_rel *= 10;
		      if (LocalOptHelper::nlopt_stop_ftol(stop, fcur, state.f0))
			  ret = NLOPT_FTOL_REACHED;
		      else if (LocalOptHelper::nlopt_stop_x(stop, xcur, state.x0))
	      ...
	      case 9: /* more than iter iterations in SQP */
              LocalOptHelper::nlopt_stop_msg(stop, "bug: more than iter SQP iterations");
		  ret = NLOPT_FAILURE;
		  goto done;
	      case 2: /* number of equality constraints > n */
	      default: /* >= 10: working space w or jw too small */
              LocalOptHelper::nlopt_stop_msg(stop, "bug: workspace is too small");
	  ...
	  if (LocalOptHelper::nlopt_isfinite(fcur) && ((fcur < *minf && (feasible_cur || !feasible))
	  ...
	  if (mode == -1) {
	       if (!LocalOptHelper::nlopt_isinf(fprev)) {
		    if (LocalOptHelper::nlopt_stop_ftol(stop, fcur, fprev))
			 ret = NLOPT_FTOL_REACHED;
		    else if (LocalOptHelper::nlopt_stop_x(stop, xcur, xprev))
			 ret = NLOPT_XTOL_REACHED;
	       }
	       fprev = fcur;
	  ...
	  if (LocalOptHelper::nlopt_stop_evals(stop)) ret = NLOPT_MAXEVAL_REACHED;
	  else if (LocalOptHelper::nlopt_stop_time(stop)) ret = NLOPT_MAXTIME_REACHED;
	  else if (feasible && *minf < stop->minf_max) ret = NLOPT_MINF_MAX_REACHED;
     } while (ret == NLOPT_SUCCESS);

done:
     if (LocalOptHelper::nlopt_isinf(*minf)) { /* didn't find any feasible points, just return last point evaluated */
	  if (LocalOptHelper::nlopt_isinf(fcur)) { /* invalid cur. point, use previous pt. */
	       *minf = fprev;
